...
void PQSortedArray::enqueue(DataPoint elem) {
    ...
    else if (elem.priority < elements[numItems-1].priority) {
        elements[numItems] = elem;
        numItems++;
    } else {

    int index = 0;
    while (index < numItems && elem.priority < elements[index].priority) {
        index++;
    }
...
void PQSortedArray::expand() {
    // allocate new space
    DataPoint* newElements = new DataPoint[2*allocatedCapacity];
...
void fill(PQSortedArray& pq, int n) {
    for (int i = 0; i < n; i++){
        pq.enqueue({ "", i });
    }
}

void empty(PQSortedArray& pq, int n) {
    for (int i = 0; i < n; i++){
        pq.dequeue();
    }
}
STUDENT_TEST("Test on size 12000") {
    PQSortedArray a;

    TIME_OPERATION(10000, fill(a, 12000));
    TIME_OPERATION(10000, empty(a, 12000));
}
STUDENT_TEST("Test on size 24000") {
    PQSortedArray b;

    TIME_OPERATION(10000, fill(b, 24000));
    TIME_OPERATION(10000, empty(b, 24000));
}

STUDENT_TEST("Test on size 48000") {
    PQSortedArray c;

    TIME_OPERATION(10000, fill(c, 48000));
    TIME_OPERATION(10000, empty(c, 48000));
}
